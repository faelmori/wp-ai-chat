    ...
    foreach ($lines as $line) {
      ...
      if ($line && strpos($line, 'data:') === 0) {
        ...
        switch ($provider) {
          ...
          case 'tencent':
            if (
              isset($json_data['choices'][0]['delta']['role']) &&
          ...
          case 'coze':
            // 处理普通消息
            if (
              isset($event) && $event === 'conversation.message.delta' &&
            ...
            // 处理function_call
            elseif (
              isset($event) && $event === 'conversation.chat.tool_call.invoke' &&
...
?>
  <style>
    ...
    .dashscope-wrap input[type="datetime-local"] {
      padding: 8px;
    ...
    #deepseek-agents-table th:nth-child(8),
    #deepseek-agents-table td:nth-child(8) {
      width: 100px;
      white-space: nowrap;
    }
    ...
    <form method="post" action="options.php">
      ...
      <div class="dashscope-section">
        ...
        <p>
          ...
          <input type="text" id="agent_file_formats" name="agent_file_formats"
            value="<?php echo esc_attr(get_option('agent_file_formats', 'pdf')); ?>" style="width: 500px;" />
        <p class="description">多种用英文逗号分隔，例如：pdf,docx,txt</p>
        </p>
        <p>
          <strong>最大文件大小（MB）</strong>
          <input type="number" id="agent_file_max_size" name="agent_file_max_size"
            value="<?php echo esc_attr(get_option('agent_file_max_size', 10)); ?>" min="1" style="width: 100px;" />
        <p class="description">设置支持上传的最大文件大小，单位为MB</p>
        </p>

        <p>支持阿里、腾讯、火山引擎和扣子平台的智能体应用。阿里API Key就是百炼里面的，腾讯需为每个智能体单独设置Token，火山引擎和模型apikey一样，扣子的个人访问令牌Token需定期更换，文件上传只支持扣子应用。
        </p>
      </div>
...
?>
  <div class="wrap">
    ...
    <form method="get" class="search-form" style="margin-bottom: 20px;">
      ...
      <input type="number" name="user_id" id="user_id" value="<?php echo esc_attr($search_user_id); ?>" min="1"
        style="width: 100px;">
...
?>
  ...
  <p>
    ...
    <input type="datetime-local" id="coze_access_token_expiry" name="coze_access_token_expiry"
    ...
    <tbody>
      <?php foreach ($agents as $index => $agent): ?>
        <tr>
          ...
          <td><input type="text" name="deepseek_agents[<?php echo $index; ?>][name]"
              value="<?php echo esc_attr($agent['name']); ?>" /></td>
          <td><input type="text" name="deepseek_agents[<?php echo $index; ?>][description]"
              value="<?php echo esc_attr($agent['description']); ?>" /></td>
          <td><input type="url" name="deepseek_agents[<?php echo $index; ?>][icon]"
              value="<?php echo esc_attr($agent['icon']); ?>" /></td>
          <td><input type="text" name="deepseek_agents[<?php echo $index; ?>][app_id]"
              value="<?php echo esc_attr($agent['app_id']); ?>" /></td>
          <td>
            <?php if ($agent['provider'] === 'tencent'): ?>
              <input type="text" name="deepseek_agents[<?php echo $index; ?>][token]"
                value="<?php echo esc_attr($agent['token'] ?? ''); ?>" style="width: 200px;" />
            <?php else: ?>
              <span>-</span>
            <?php endif; ?>
          </td>
