...
function deepseek_announcement_callback()
{
  ...
  ?>
  <textarea name="deepseek_announcement" rows="5" cols="50"
    style="width: 400px;"><?php echo esc_textarea($announcement); ?></textarea>
...
function max_file_size_callback()
{
  ...
  ?>
  <input type="number" name="max_file_size" value="<?php echo esc_attr($size); ?>" min="1" max="500"
    style="width: 100px;" />
      ...
      <div id="deepseek-options-bar">
        ...
        <?php if ($qwen_enable_search == '1' && in_array($current_interface, array_keys($search_supported_models))): ?>
          <div class="deepseek-option-item deepseek-search-toggle" style="display: none;"
            data-supported-models='<?php echo json_encode($search_supported_models); ?>'>
            <label class="switch">
...
function deepseek_delete_log()
{
  ...
  // 检查是否为管理员
  if (
    !current_user_can('manage_options') && !$wpdb->get_var($wpdb->prepare(
            ...
            <tr>
              ...
              <td>
                <a href="?page=deepseek-logs&delete_conversation=<?php echo esc_attr($log->conversation_id); ?><?php echo $search_user_id ? '&user_id=' . $search_user_id : ''; ?>"
                  class="button" onclick="return confirm('确定要删除此对话记录吗？');">删除</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
...
function deepseek_generate_article_stream_ajax()
{
  ...
  if (empty($api_key) || empty($model) || empty($url)) {
    echo "data: " . json_encode([
      'error' => '接口配置缺失 - API Key: ' . ($api_key ? '已设置' : '未设置') .
        ', Model: ' . ($model ? $model : '未设置') .
        ', URL: ' . ($url ? $url : '未设置')
    ]) . "\n\n";
    flush();
